...
import { ArrowLeft, RefreshCw, Trash2, Check, Circle } from "lucide-react"
    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-hairline bg-background/80 backdrop-blur-md">
        <nav className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-6 py-4 sm:px-10">
          ...
          {!authed && <span className="w-16" />}
        </nav>
      </header>

      <main className="mx-auto max-w-6xl px-6 py-16 sm:px-10">
        {!authed ? (
          <form onSubmit={login} className="mx-auto flex max-w-md flex-col gap-6 border border-hairline bg-muted/30 p-8">
            <div>
              <p className="font-mono text-[10px] uppercase tracking-[0.28em] text-muted-foreground">Acceso privado</p>
              <h1 className="mt-3 font-display text-3xl leading-tight text-foreground">Dashboard de leads</h1>
              <p className="mt-4 text-sm leading-relaxed text-muted-foreground">
                Ingresa el <code className="font-mono text-[13px] text-foreground">ADMIN_PASSWORD</code> configurado en Vercel env vars.
              </p>
            </div>
            <input
              type="password"
            ...
            <button type="submit" disabled={loading} className="btn-plain btn-plain-inv w-fit gap-2 disabled:opacity-40">
          ...
          <div className="flex flex-col gap-6">
            <div className="flex flex-wrap items-baseline justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  type="button"
                  onClick={() => void fetchLeads(password || sessionStorage.getItem(STORAGE_KEY) || "")}
                  disabled={loading}
                  className="flex items-center gap-2 border border-hairline px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.24em] text-foreground transition-colors hover:bg-muted disabled:opacity-40"
                >
            ...
            {warning && (
              <p className="border-l-2 border-amber-400 bg-amber-400/5 p-4 text-sm text-amber-200">
                ⚠️ {warning}. Ve a Vercel Dashboard → Storage → Create Database → Postgres, vincúlalo al proyecto y redeploy.
              </p>
            )}
                ...
                return (
                  <div
                    ...
                    className={`border p-5 transition-colors ${
                      lead.contacted ? "border-hairline bg-transparent opacity-70" : "border-foreground/30 bg-muted/40"
                    }`}
                  >
                    <div className="flex flex-wrap items-baseline justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="flex items-center gap-2 border border-hairline px-2.5 py-1 font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground transition-colors hover:border-foreground hover:text-foreground"
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 border border-hairline px-2 py-1 font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground transition-colors hover:border-red-400 hover:text-red-400"
                          title="Eliminar"
                        >
                    ...
                    {lead.client_brief && (
                      <div className="mt-4">
                        ...
                        <p className="mt-2 border-l-2 border-foreground/60 pl-4 text-[15px] leading-[1.65] text-foreground/95">
                          {lead.client_brief}
                        </p>
                      </div>
                    )}
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-4 border-t border-hairline pt-4">
                      ...
                      <button
                        ...
                        className="ml-auto font-mono text-[10px] uppercase tracking-[0.24em] text-muted-foreground transition-colors hover:text-foreground"
                      >
                        {isOpen ? "Ocultar resumen" : "Ver resumen completo"}
                      </button>
                    </div>

                    {isOpen && lead.summary && (
                      <pre className="mt-3 max-h-64 overflow-auto whitespace-pre-wrap border border-hairline bg-background/60 p-4 font-mono text-[12px] leading-[1.6] text-foreground/90">
                        {lead.summary}
                      </pre>
                    )}
                    {isOpen && (
